...
def main():
	parser = argparse.ArgumentParser(prog='Obfuscator.py')
	parser.add_argument("ip", help="IP Which You Want To Convert")
	...
	print(end+G+Bold+"𝕀ℙ 𝕆𝕓𝕗𝕦𝕤𝕔𝕒𝕥𝕠𝕣 made with "+heart+" By  𝖌𝖍𝖔𝖘𝖙𝖍𝖚𝖇 ", file=sys.stderr)
	ip = args.ip
	if ip.count(".")!=3:
		print(end+R+Bold+"Sorry, We Convert IP's Only Not URL's!"+end, file=sys.stderr)
		exit(0)
